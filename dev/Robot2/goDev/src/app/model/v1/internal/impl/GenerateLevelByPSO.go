...
type LevelGeneByPSO struct {
	Map     [][]int
	Units   map[[2]int]string
	Fitness float64
}

func (this LevelGeneByPSO) CalcFitness() (optalg.IGene, error) {
	if _count == nil {
...
func (this LevelGeneByPSO) GetFitness() float64 {
	return this.Fitness
}
func (this LevelGeneByPSO) Crossover(b optalg.IGene) (optalg.IGene, error) {
	units := map[[2]int]string{}
	...
	case 0:
		for y := 0; y < mapH; y++ {
			for x := 0; x < mapW; x += 2 {
				...
				if _, has := b.(LevelGeneByPSO).Units[pos2]; has {
					units[pos2] = b.(LevelGeneByPSO).Units[pos2]
				}
			}
		}
	case 1:
...
func (this LevelGeneByPSO) Mutate() (optalg.IGene, error) {
