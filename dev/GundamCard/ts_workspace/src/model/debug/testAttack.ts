...
export function testAttack() {
  ...
  const unit1 = ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "ground" })]?.[0]
  if (unit1 == null) {
    throw new Error("unit1 not found")
  }
  const unit2 = ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "ground" })]?.[1]
  if (unit2 == null) {
  ...
  if ((ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "earth" })]?.length || 0) != 1) {
  ...
  if ((ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "earth" })]?.length || 0) != 2) {
    throw new Error("地球區必須有2機")
  }
  const unitOnEarth1 = ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "earth" })]?.[0]
  if (unitOnEarth1 == null) {
    throw new Error("unitOnEarth1 not found")
  }
  const unitOnEarth2 = ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerA, where: "earth" })]?.[1]
  ...
  const enemy1 = ctx.gameState.table.cardStack[cardPositionID({ playerID: PlayerB, where: "ground" })]?.[0]
  if (enemy1 == null) {
