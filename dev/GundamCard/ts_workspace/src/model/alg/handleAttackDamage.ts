...
export function handleAttackDamage(ctx: Context, attackPlayerID: string, guardPlayerID: string, where: CardBattleWhere, speed: AttackSpeed): Context {
    const attackUnits = ctx.gameState.table.cardStack[cardPositionID({ playerID: attackPlayerID, where: where })] || []
    const attackPower = attackUnits.map((card, i): number => {
        // 破壞的單位沒有攻擊力
        let live = 0
    ...
    const guardUnits = ctx.gameState.table.cardStack[cardPositionID({ playerID: guardPlayerID, where: where })] || []
    const guardPower = guardUnits.map((card, i): number => {
        // 破壞的單位沒有攻擊力
        let live = 0
        mapCardState(ctx, [card.id], cardState => {
            live = cardState.live
    ...
    if (askAttackSpeedEnable(attackUnits, speed)) {
        ...
        ctx = mapCardState(ctx, guardUnits.map(unit => unit.id), cardState => {
    ...
    if (askAttackSpeedEnable(guardUnits, speed)) {
        ...
        ctx = mapCardState(ctx, attackUnits.map(unit => unit.id), cardState => {
