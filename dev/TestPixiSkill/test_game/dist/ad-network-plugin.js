window.adNetworkPlugin = function () {
  ...
  var ctaPressed2 = /* @__PURE__ */ __name(() => {
    ...
    console.warn("vungle CTA pressed!");
  }, "ctaPressed");
  var gameReady = /* @__PURE__ */ __name(() => {
    window.gameReady && window.gameReady();
    console.warn("vungle Game ready!");
  }, "gameReady");
  function gameStart() {
    console.warn("vungle game start");
  }
  ...
  function gameClose(gameClose2) {
    console.warn("vungle game closed");
    gameClose2();
  }
  ...
  var onSdkReady = /* @__PURE__ */ __name((initGame) => {
    mraid.addEventListener("viewableChange", (viewable) => {
      viewableChangeHandler(viewable, initGame)
    });
    if (mraid.isViewable()) {
      showMyAd(initGame);
    }
  ...
  var viewableChangeHandler = /* @__PURE__ */ __name((viewable, initGame) => {
    if (viewable) {
      showMyAd(initGame);
    } else {
    }
  ...
  var maioNetwork = {
    init(fn) {
      if (typeof Maio === "undefined") {
        console.warn("Maio not found!");
        fn()
      } else {
    ...
    ctaPressed(googlePlayStore, appleStore) {
      if (typeof Maio === "undefined") {
        console.warn("Maio called in development environment!");
      } else {
        if (googlePlayStore !== "" && appleStore !== "") {
          const url = getPlatform(googlePlayStore, appleStore);
          Maio.openClickUrl(url)
        } else {
          console.warn("googlePlayStore and appleStore parameters are empty!");
        }
      }
  ...
  var bgyMraidNetwork = {
    ctaPressed: () => {
      if (typeof window.BGY_MRAID === "undefined") {
        console.warn("BGY_MRAID not found!");
      } else {
  ...
  var MaioNetwork = maioNetwork;
  var BgyMraid = bgyMraidNetwork;
  return {
    AdNetworkFactory,
    Google,
    Meta,
