...
import { setTipSelectionForUser, doEffect } from "../game/gameState/effect"
...
export async function testIssue() {
    ...
    let ctx = ISSUE_CTX2
    const cmds = getPlayEffects(ctx, PlayerA)
    if (cmds.length == 0) {
        throw new Error()
    }
    const effect = cmds[1]
