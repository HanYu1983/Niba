  ...
  texts: [
    {
                        ...
                        title: ["cutIn", [
                          {
                            title: function _(ctx: GameState, effect: Effect, { DefineFn, GameStateFn, Options }: Bridge): GameState {
                              ...
                              for (const pair of pairs) {
                                ctx = GameStateFn.doItemMove(ctx, effect, GameStateFn.getItemBaSyou(ctx, cardId), pair, { insertId: 0, ges: Options.ges }) as GameState
                              }
                              return ctx
                            }.toString()
                          }
      ...
      onEvent: function _(ctx: GameState, effect: Effect, { DefineFn, GameStateFn, Options }: Bridge): GameState {
        ...
        if (event.title[0] == "GameEventOnTiming" && DefineFn.PhaseFn.eq(event.title[1], DefineFn.PhaseFn.getLast())) {
          ...
          for (const pair of pairs) {
            ctx = GameStateFn.doItemMove(ctx, effect, DefineFn.AbsoluteBaSyouFn.of(cardOwner, "手札"), GameStateFn.createStrBaSyouPair(ctx, pair[0]), { ges: Options.ges }) as GameState
          }
        }
        return ctx
      }.toString(),
