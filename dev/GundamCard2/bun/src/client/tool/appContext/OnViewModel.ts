    ...
    try {
      switch (evt.id) {
        case "OnClickNewGame": {
          ...
          ctx.gameState = createCardWithProtoIds(ctx.gameState, AbsoluteBaSyouFn.of(PlayerA, "Gゾーン"), deckA.slice(6,12)) as GameStateWithFlowMemory
          ctx.gameState = createCardWithProtoIds(ctx.gameState, AbsoluteBaSyouFn.of(PlayerA, "配備エリア"), deckA.slice(12, 18)) as GameStateWithFlowMemory
          ctx.gameState = createCardWithProtoIds(ctx.gameState, AbsoluteBaSyouFn.of(PlayerB, "Gゾーン"), deckB.slice(0, 6)) as GameStateWithFlowMemory
          ctx.gameState = createCardWithProtoIds(ctx.gameState, AbsoluteBaSyouFn.of(PlayerB, "配備エリア"), deckB.slice(12, 18)) as GameStateWithFlowMemory
          // ctx.gameState = {
          //   ...ctx.gameState,
