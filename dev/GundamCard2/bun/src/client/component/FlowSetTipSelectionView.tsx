...
import { StrBaSyouPair, Tip, TipFn, TipTitleTextRef } from "../../game/define/Tip";
...
import { getCardTextFromCardTextRef } from "../../game/gameState/card";
...
export const FlowSetTipSelectionView = (props: { clientId: string, flow: FlowSetTipSelection, style?: CSSProperties }) => {
  ...
  const renderWant = useMemo(() => {
                ...
                onClick={()=>{
                  const tip: Tip = {
                    title: [
                      ...
                      TipFn.getWant(props.flow.tip) as any[],
                      [value]
                    ],
                  }
                  OnEvent.next({
    ...
    case "テキスト":{
      const values = wants as TipTitleTextRef[]
      return <div style={{
        display: "flex",
        border: "2px solid black",
        overflow: "scroll",
              ...
              <button 
                onClick={()=>{
                  const tip: Tip = {
                    title: [
                      "テキスト",
                      TipFn.getWant(props.flow.tip) as TipTitleTextRef[],
                      [value]
                    ],
                  }
                  OnEvent.next({
