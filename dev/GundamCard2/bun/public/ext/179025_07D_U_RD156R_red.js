    ...
    texts: [
        {
            ...
            onEvent: function _(ctx, effect, { DefineFn, GameStateFn, Options }) {
                ...
                if (evt.title[0] == "カット終了時" && hasSpecialPlayX) {
                    ctx = GameStateFn.doItemMove(ctx, effect, DefineFn.AbsoluteBaSyouFn.of(cardController, "ジャンクヤード"), GameStateFn.createStrBaSyouPair(ctx, cardId), { ges: Options.ges });
                }
                return ctx;
            }.toString()
        },
                            ...
                            actions: [
                                {
                                    title: function _(ctx, effect, { DefineFn, GameStateFn, Options }) {
                                        ...
                                        for (const targetE of targetEs) {
                                            ctx = GameStateFn.removeEffect(ctx, targetE.id);
                                        }
                                        for (const pair of pairs) {
                                            ctx = GameStateFn.doItemMove(ctx, effect, DefineFn.AbsoluteBaSyouFn.of(cardController, "ジャンクヤード"), pair, { ges: Options.ges });
                                        }
                                        return ctx;
                                    }.toString(),
                                }
