    ...
    texts: [
        {
                                                ...
                                                title: ["cutIn", [
                                                        {
                                                            title: function _(ctx, effect, { DefineFn, GameStateFn, Options }) {
                                                                ...
                                                                for (const pair of pairs) {
                                                                    ctx = GameStateFn.doItemMove(ctx, effect, GameStateFn.getItemBaSyou(ctx, cardId), pair, { insertId: 0, ges: Options.ges });
                                                                }
                                                                return ctx;
                                                            }.toString()
                                                        }
            ...
            onEvent: function _(ctx, effect, { DefineFn, GameStateFn, Options }) {
                ...
                if (event.title[0] == "GameEventOnTiming" && DefineFn.PhaseFn.eq(event.title[1], DefineFn.PhaseFn.getLast())) {
                    ...
                    const pairs = DefineFn.TipFn.getSelection(tip);
                    if (pairs.length == 0) {
                        return ctx;
                    }
                    for (const pair of pairs) {
                        ctx = GameStateFn.doItemMove(ctx, effect, DefineFn.AbsoluteBaSyouFn.of(cardOwner, "手札"), GameStateFn.createStrBaSyouPair(ctx, pair[0]), { ges: Options.ges });
                    }
                }
                return ctx;
            }.toString(),
