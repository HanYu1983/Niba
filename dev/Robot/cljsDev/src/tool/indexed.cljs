...
(defn make-indexed [key-fn else-fn]
  (sorted-set-by
   (fn [o1 o2]
     (let [c (compare (key-fn o1)
       ...
       (if (not= 0 c)
         c
         (compare (else-fn o1) (else-fn o2)))))))


(defn sync-indexed [key-fn value-fn remove-fn create-fn update-fn values key-fn2 origin]
  (let [next origin
        now-entities (zipmap (map key-fn values)
                             (map value-fn values))
        
        old-ids (->> (map key-fn2 next) (into #{}))
