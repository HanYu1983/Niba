...
| Buff buffCatalog | 38 | 19 個解譯欄位（屬性修正/乘區/地形/復活…） |
| 功法 | 36 | 內功 7（傷害公式閉包）、傷害外功 7、功能外功 22 |
| 裝備 equipmentCatalog | 39 | **純五維 modifiers**，無特殊效果 |
| 探索事件 eventCatalog | 31 型／約 60 選項 | `EventEffect` 9 種＋`EventRequirement` 6 種 |
| 建築 buildingCatalog | 18 | bonus 欄位 3 種＋actions 3 種＋特例邏輯 |
...
| 語義 | 數量 | 代表 |
|---|---|---|
| 移動/地形消耗 | 12 | 六步功、swift-wind-movement、主場×5 的移動部分 |
| 屬性修正 | 7 | 五維常駐×5、home-turf-water/ruin 的加值 |
| 傷害乘區 | 6 | break-army/vigor、主場三兄弟的 +15%、共鳴迴避 |
| 減益/控場 | 6 | burning×2、cold-poison×2、immobilize、reflection×2 |
...
### 2.3 功能外功（22 種 effect，全部已接 Buff）
| 語義 | 數量 |
|---|---|
| 移動/地形適應 | 7（terrain-adaptation＋六步） |
| 資源吸取/週期回復 | 4 |
| 傷害增益 | 3（critical/damage-dealt/external-damage） |
| 條件爆發 | 3 |
| 攻擊附著減益 | 2（burning/attribute-reduction） |
...
## 3. 跨載體效果家族總表（合計約 205 個效果承載條目）

占比為跨所有載體的粗估，用於判斷飽和度。

| # | 效果家族 | 約數 | 占比 | 主要承載者 |
|---|---|---|---|---|
| F1 | 屬性修正／成長 | ~53 | **25%** | 裝備 39、屬性丹 5、五維常駐 Buff 7、經驗加成功法 |
| F2 | 傷害輸出 | ~44 | 22% | 元素爆發道具 24、傷害功法 14、傷害乘區 Buff、箭塔 3 |
