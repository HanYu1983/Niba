...
| 攻擊只選最近生物 | `fuzzyInputs.ts` | 未評估哪隻最值得打（經驗值 / 威脅度 / 可擊殺性） |
...
| 2 | 攻擊目標候選化 | 從「最近」改為「枚舉視野內所有生物 → 各自算價值」 |
...
| 攻擊目標候選化（枚舉視野內生物 → 各自算價值） | P1 | AI | — | 不再只選最近；可擊殺 + 高經驗者優先 |
...
| M3：價值可解釋性 | `[AI decision]` 日誌含各價值維度分解 | 日誌檢查 | 未開始 |
