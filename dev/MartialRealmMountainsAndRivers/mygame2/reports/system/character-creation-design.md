...
>
> 原「固定點數創角＋背景天賦」方案（§1–§11）已演進為 **「跨對局角色養成（卷系統）」**：角色以**名册形式持久存在**，每局結束依表現結算 **培養卷**，用於永久提升五維與設定初始功法。本文件以 **§12** 為現行定案設計，§1–§11 保留為舊方案參考（其固定點數分配與背景天賦暫不採用）。
>
