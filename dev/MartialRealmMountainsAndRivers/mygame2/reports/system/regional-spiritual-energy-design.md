...
- **巢穴靈氣 (Nest Auras)**
    - **來源**：敵方巢穴。
    - **關聯性**：巢穴的「主導元素」決定其產生的區域靈氣類型。
    - **效果**：
        - **對玩家**：產生負面環境效果（詳情請參閱 `reports/system/nest-mechanics-design.md`）。例如：火焰巢穴產生「熾熱靈氣」，對範圍內玩家造成持續火焰傷害或降低移動速度。
        - **對怪物**：產生正面效果，例如提升怪物傷害、防禦或移動速度。
        - **破解機制**：玩家可透過特定道具或技能「破解」巢穴靈氣，解除負面效果並對巢穴造成大量傷害。
...
- **玩家破解機制**：
    - **觸發條件**：玩家使用特定道具（例如「淨化符」或「破陣丹」）或技能，在巢穴靈氣範圍內進行「破解」動作。
    - **效果**：
        - **解除靈氣**：暫時或永久移除該巢穴的區域靈氣效果。
        - **造成傷害**：對巢穴本身造成一次顯著的傷害（例如，最大生命值的 10-20%）。
    - **冷卻機制**：破解動作可能需要冷卻時間，或消耗稀有資源，以避免濫用。

### 3.3 靈氣的影響範圍與強度

-   **範圍 (Radius)**：每個靈氣源都有一個影響範圍，通常以其為中心呈圓形或方形擴散。
-   **強度 (Strength)**：靈氣的效果強度可以由來源決定，或根據玩家與來源的距離衰減。
-   **巢穴靈氣的特殊性**：巢穴靈氣的強度可能與其當前生命值、生成的怪物數量或巢穴等級相關。破解後，靈氣強度可能暫時歸零或大幅減弱。
