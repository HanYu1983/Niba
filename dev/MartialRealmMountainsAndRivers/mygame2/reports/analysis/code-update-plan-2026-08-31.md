...
| 6 結構拆分 | 🔄 **進行中** | 6.1 部分完成：`store/createStore.ts`、`session/sessionController.ts`、`effects/animationBus.ts`、`ai/aiStepRunner.ts`、`actions/itemBurstActions.ts` 已抽出；純規則轉出口已移出 store；`gameStore.ts` 由 2359 行降至 1519 行；6.2 ⬜ 未開始 |
