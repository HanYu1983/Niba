...
## 2026-09-06｜資源點擊破地圖提示

### 本次完成

- **新增資源點失活提示 icon**：資源點被 Creature 摧毀後，保留在原地圖格並以灰階樣式顯示，避免玩家誤認為仍可採集。
- **新增修復警示徽章**：當資源點 `active === false` 時，在地圖 marker 右上角顯示黃色 `!` 徽章，提示玩家該資源點需要修復。
- **保留既有互動資訊**：失活資源點仍可點擊查看詳情，tooltip 與 aria-label 持續顯示「已失活，請修復」；正常資源點的 icon、樣式與採集流程不變。

### 影響檔案

- 修改：`src/components/MapGrid.tsx`（失活資源點 marker class 與警示徽章）
- 修改：`src/App.css`（失活灰階樣式與 `!` 警示徽章樣式）

### 驗證結果

- TypeScript：通過。
- Vite production build：通過（`npm run build`）。
- 修改檔案診斷：無錯誤。

### 下一步

- 確認實際遊戲中資源點被 Creature 摧毀後，地圖會即時顯示灰階 icon 與黃色 `!` 徽章。
- 確認玩家點擊失活資源點後，可從詳情彈窗執行修復。
