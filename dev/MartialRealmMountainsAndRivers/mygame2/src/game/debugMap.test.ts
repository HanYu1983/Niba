...
describe('createDebugGameState', () => {
  ...
  it('將主要互動物件放在玩家附近', () => {
    ...
    expect(new Set(state.creatures.map((creature) => creature.schoolId))).toEqual(new Set([
      'frost-water', 'golden-body', 'swift-wind', 'scarlet-flame', 'earth-mountain', 'void-spirit', 'hundred-poison', 'ghost-shadow',
    ]))
  ...
  it('提供可直接測試的玩家資源與外功', () => {
    ...
    expect(state.bases[0].buildings).toHaveLength(18)
  ...
  it('包含不同門派的門派據點供測試（不再有等級差異）', () => {
    const state = createDebugGameState()
    expect(state.sectGates ?? []).toHaveLength(8)
    expect(new Set((state.sectGates ?? []).map((gate) => gate.schoolId))).toEqual(new Set([
      'golden-body', 'swift-wind', 'scarlet-flame', 'frost-water', 'earth-mountain', 'void-spirit', 'hundred-poison', 'ghost-shadow',
    ]))
