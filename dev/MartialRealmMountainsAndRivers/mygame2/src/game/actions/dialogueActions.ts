...
/**
 * 將對話步驟轉為佇列項，並推入 campaignState.dialogueQueue。
 * 若 state 沒有 campaignState（沙盒模式），則回傳原 state（不污染沙盒）。
 */
...
/**
 * 取出對話佇列首項並移除之；佇列為空時回傳 null。
 * 不修改 triggeredDialogueIds（由 markDialogueTriggered 另行處理）。
 */
export function dequeueDialogue(state: GameState): { state: GameState; entry: DialogueQueueEntry | null } {
  ...
  return {
    state: {
      ...state,
      campaignState: { ...state.campaignState, dialogueQueue: rest },
    },
    entry,
  }
}

/** 將指定 stepId 標記為已觸發（避免重複觸發）。 */
export function markDialogueTriggered(state: GameState, stepId: string): GameState {
  ...
  return {
    ...
    campaignState: {
      ...state.campaignState,
      triggeredDialogueIds: [...triggered, stepId],
    },
  }
}
...
/** 清空對話佇列，並將佇列中所有 stepId 標記為已觸發（跳過全部）。 */
