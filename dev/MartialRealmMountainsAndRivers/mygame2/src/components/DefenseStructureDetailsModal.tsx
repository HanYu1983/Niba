...
function DefenseStructureDetailsModal({ structure, onOpenTransport, onClose }: DefenseStructureDetailsModalProps) {
  return (
    <Modal
      ...
      {structure && (
        <Flex vertical gap={16}>
          <List
            ...
            renderItem={(currentStructure) => (
              <BuildingListItem
                ...
                tags={
                  <>
                    <Tag color="green">生命 {currentStructure.health}/{currentStructure.maxHealth}</Tag>
          ...
          <StatBar
            ...
            max={structure.maxHealth}
            status={structure.health <= 0 ? 'exception' : 'active'}
          />

          {structure.attackRange > 0 && (
            <Typography.Text>
              自動攻擊：Creature 回合攻擊 {structure.attackRange} 格內目標，造成 {structure.attackDamage} 點傷害。
            </Typography.Text>
          )}
