    ...
    <Flex vertical gap={12}>
      ...
      <List
        ...
        renderItem={(entry) => <List.Item actions={[
          ...(entry.slot === 0 ? [] : [
            <Button key="save" type="primary" onClick={() => onSave(entry.slot)}>儲存至此欄位</Button>,
          ]),
          ...
          <List.Item.Meta title={entry.slot === 0 ? '自動存檔' : `存檔欄位 ${entry.slot}`} description={entry.savedAt ? `第 ${entry.round ?? 0} 回合｜${new Date(entry.savedAt).toLocaleString()}` : '尚未產生自動存檔'} />
        </List.Item>}
      />
