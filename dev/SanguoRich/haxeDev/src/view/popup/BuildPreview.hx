...
class BuildPreview extends PopupView{
    ...
    function onBtnConfirmClick(e:MouseEvent) {
        ...
        final to = switch(current){
            case TREASURE(level):
                if(level < 1) BUILDING.TREASURE(level+1);
                else BUILDING.TREASURE(1);
            case FISHING(level):
            ...
            case SIEGEFACTORY(level):
                if(level < 1) BUILDING.SIEGEFACTORY(level+1);
                else BUILDING.SIEGEFACTORY(1);
            case ACADEMY(level):
                if(level < 1) BUILDING.ACADEMY(level+1);
                else BUILDING.ACADEMY(1);
            case WALL(level):
