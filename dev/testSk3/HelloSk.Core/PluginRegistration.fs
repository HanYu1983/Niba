...
/// 將 Core 提供的 Plugin（通用工具、程式編輯、AWS、實際任務、RAG、Facebook、Google Ads、Ricoh 監控）註冊到 Kernel，供 AI 透過 function calling 呼叫。
module PluginRegistration =
    /// 註冊 Tools、CodeHelper、Aws、RealTasks、Rag、Facebook、GoogleAds、RicohMonitoring 到指定 kernel。
    let registerCorePlugins (kernel: Kernel) =
        ...
        kernel.Plugins.Add(kernel.CreatePluginFromObject(RealTasksPlugin(), "RealTasks"))
        kernel.Plugins.Add(kernel.CreatePluginFromObject(RagPlugin(), "Rag"))
