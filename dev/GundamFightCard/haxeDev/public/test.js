...
Test.test1 = function() {
	...
	console.log("src/Test.hx:23:","============ getGame ============");
	console.log("src/Test.hx:25:",game.ctx);
	console.log("src/Test.hx:27:","============ testMemonto ============");
	var loadGame = model_ver1_game_Game.ofMemonto(game.getMemonto());
	console.log("src/Test.hx:30:",loadGame.ctx);
	console.log("src/Test.hx:31:","=================================");
	...
	while(_g_current < _g_length) {
		...
		var key1 = _g1_key;
		var value = _g1_value;
		console.log("src/Test.hx:33:",key1);
		console.log("src/Test.hx:34:",value);
	}
	console.log("src/Test.hx:36:","=================================");
	game = loadGame;
};
...
function model_ver1_data_CardProto_$179001_$01A_$CH_$WT007R_$white_test() {
	...
	var infos = result;
	console.log("src/model/ver1/data/CardProto_179001_01A_CH_WT007R_white.hx:94:",infos);
	if(infos.length == 0) {
...
function model_ver1_data_CardProto_$179003_$01A_$U_$BK008U_$black_test() {
	...
	var infos = result;
	console.log("src/model/ver1/data/CardProto_179003_01A_U_BK008U_black.hx:88:",infos);
	if(infos.length == 0) {
		throw new haxe_Exception("infos.length == 0");
	}
	var selectTextId = infos[0].text.id;
...
model_ver1_data_CardProto_$179004_$01A_$CH_$WT009R_$white.prototype = $extend(model_ver1_game_define_CardProto.prototype,{
	getTexts: function(ctx,runtime) {
		return [new model_ver1_data_PlayerPlayCard("" + runtime.getCardId() + "_PlayerPlayCard")];
	}
	,getCLID: function() {
...
var model_ver1_data_RequireGTap = function(id,colors,ctx,runtime) {
	...
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		var cardColors = model_ver1_game_alg_Context_getCardGSign(ctx,v).colors;
		if(Lambda.fold(cardColors,function(c,a) {
			if(!a) {
...
function model_ver1_game_alg_Context_getCardType(ctx,cardId) {
	return model_ver1_game_define_CardCategory.Unit;
}
...
function model_ver1_game_alg_Context_getCardGSign(ctx,cardId) {
	return { colors : [model_ver1_game_define_GColor.Red], production : ""};
}
...
function model_ver1_game_alg_Context_getCard(ctx,cardId) {
	var card = ctx.table.cards.h[cardId];
	if(card == null) {
...
function model_ver1_game_alg_Runtime_getRuntimeText(ctx) {
	...
	while(_g2 < _g3.length) {
		...
		++_g2;
		var tmp;
		var _g = model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,v.id);
		var _g4 = _g.playerId;
	...
	var cardsNotHome = _g1;
	var _g = [];
	var _g1 = 0;
	var _g2 = cardsNotHome;
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		var tmp;
		var _g3 = model_ver1_game_alg_Context_getCardController(ctx,v.id);
		if(_g3._hx_index == 0) {
			var _g4 = _g3.v;
			tmp = false;
		} else {
	...
	var cardsHasNoController = _g;
	var _g = [];
	var _g1 = 0;
	var _g2 = cardsHasNoController;
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		var tmp;
		var _g3 = model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,v.id);
		var _g4 = _g3.playerId;
		switch(_g3.baSyouKeyword._hx_index) {
		case 7:case 8:
			tmp = true;
			break;
		default:
	...
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		if(cardsInHandAndHanger.indexOf(v) != -1 == false) {
			_g.push(v);
		}
	}
	var cardsNotInHandAndHanger = _g;
	var _g = [];
	var _g1 = 0;
	var _g2 = cardsNotInHandAndHanger;
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		var _g3 = model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,v.id);
		var _g4 = _g3.playerId;
		if(_g3.baSyouKeyword._hx_index == 5) {
			_g.push(v);
		}
	}
	var cardsUseG = _g;
	var _g = [];
	var _g1 = 0;
	while(_g1 < cardsUseG.length) {
		var card = cardsUseG[_g1];
		++_g1;
		var responsePlayerId = model_ver1_game_alg_Context_getBaSyouControllerAndAssertExist(ctx,model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,card.id));
		var runtime = new model_ver1_game_define_DefaultExecuteRuntime(card.id,responsePlayerId);
		var _g2 = 0;
	...
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		if(cardsUseG.indexOf(v) != -1 == false) {
			_g.push(v);
		}
	}
	var cardsNotUseG = _g;
	var _g = [];
	var _g1 = 0;
	while(_g1 < cardsNotUseG.length) {
		var card = cardsNotUseG[_g1];
		++_g1;
		var responsePlayerId = model_ver1_game_alg_Context_getBaSyouControllerAndAssertExist(ctx,model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,card.id));
		var runtime = new model_ver1_game_define_DefaultExecuteRuntime(card.id,responsePlayerId);
		var _g2 = 0;
	...
	while(_g1 < _g2.length) {
		var v = _g2[_g1];
		++_g1;
		if(cardsHasNoController.indexOf(v) != -1 == false) {
			_g.push(v);
		}
	}
	var cardsHasController = _g;
	var _g = [];
	var _g1 = 0;
...
function model_ver1_game_define_BaSyou_getCardStackId(obj) {
	return tool_Helper_getMemontoWithWrapper(obj);
}
function model_ver1_game_define_BaSyou_getBaSyou(cardStackId) {
	return tool_Helper_ofMemontoWithWrapper(cardStackId);
}
...
var tool__$Helper_Wrapper = function() {
	this.__uid = hxbit_Serializer.SEQ << 24 | ++hxbit_Serializer.UID;
};
$hxClasses["tool._Helper.Wrapper"] = tool__$Helper_Wrapper;
tool__$Helper_Wrapper.__name__ = "tool._Helper.Wrapper";
tool__$Helper_Wrapper.__interfaces__ = [hxbit_Serializable];
tool__$Helper_Wrapper.prototype = {
	getCLID: function() {
		return tool__$Helper_Wrapper.__clid;
	}
	,serialize: function(__ctx) {
		__ctx.addDynamic(this.hold);
	}
	,getSerializeSchema: function() {
		var schema = new hxbit_Schema();
		schema.fieldsNames.push("hold");
		schema.fieldsTypes.push(hxbit_PropTypeDesc.PDynamic);
		schema.isFinal = hxbit_Serializer.isClassFinal(tool__$Helper_Wrapper.__clid);
		return schema;
	}
	,unserializeInit: function() {
	}
	,unserialize: function(__ctx) {
		this.hold = __ctx.getDynamic();
	}
	,__class__: tool__$Helper_Wrapper
};
...
function tool_Helper_getMemontoWithWrapper(obj) {
	var wrapper = new tool__$Helper_Wrapper();
	wrapper.hold = obj;
	return tool_Helper_getMemonto(wrapper);
}
function tool_Helper_ofMemontoWithWrapper(memonto) {
	var wrapper = tool_Helper_ofMemonto(memonto,tool__$Helper_Wrapper);
	return wrapper.hold;
}
...
tool__$Helper_Wrapper.__clid = hxbit_Serializer.registerClass(tool__$Helper_Wrapper);
