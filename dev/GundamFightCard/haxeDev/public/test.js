...
function model_ver1_data_CardProto_$179001_$01A_$CH_$WT007R_$white_test() {
	...
	var infos = result;
	console.log("src/model/ver1/data/CardProto_179001_01A_CH_WT007R_white.hx:101:",infos);
	if(infos.length == 0) {
...
model_ver1_data__$CardProto_$179004_$01A_$CH_$WT009R_$white_Text1_$1.prototype = $extend(model_ver1_game_define_CardText.prototype,{
	getRequires: function(ctx,runtime) {
		...
		while(_g2 < _g3.length) {
			var v = _g3[_g2];
			++_g2;
			if(gainCardSetGroupsIds.indexOf(v.id) != -1 == false && model_ver1_game_alg_Context_isMyCard(ctx,_gthis.gainCardId,v.id)) {
				_g1.push(v);
			}
		}
		var _this = _g1;
		...
		if(event._hx_index == 3) {
			...
			if(rollCardId == thisCardId) {
				...
				while(_g1 < _g2.length) {
					...
					if(_g3._hx_index == 0 && _g3.v._hx_index == 3) {
						_g.push(v);
					}
				}
				var _g1 = [];
				...
				var unitIds = _g1;
				if(unitIds.length >= 1) {
					var block = new model_ver1_game_define_Block(this.getSubKey(0),model_ver1_game_define_BlockCause.TextEffect(thisCardId,this.id),new model_ver1_data__$CardProto_$179030_$11E_$CH_$BN091N_$brown_Process1("" + this.id + "_Process1",unitIds));
					block.isImmediate = true;
					model_ver1_game_alg_Cut_cutIn(ctx,block);
				}
			}
...
var model_ver1_data__$CardProto_$179030_$11E_$CH_$BN091N_$brown_Process1 = function(id,tips) {
...
model_ver1_data__$CardProto_$179030_$11E_$CH_$BN091N_$brown_Process1.prototype = $extend(model_ver1_game_define_CardText.prototype,{
	getRequires: function(ctx,runtime) {
		var req = new model_ver1_data_RequireUserSelectCard(this.getSubKey(0),"敵軍G１枚");
		req.tips = this.tips;
		return [req];
	}
	,action: function(ctx,runtime) {
		...
		while(_g < _g1.length) {
			var cardId = _g1[_g];
			++_g;
			var mark = new model_ver1_game_define_CanNotRerollMark(this.getSubKey(0),cardId);
			mark.age = 2;
			ctx.marks.h[mark.id] = mark;
		}
	}
	...
	,getSerializeSchema: function() {
		var schema = model_ver1_game_define_CardText.prototype.getSerializeSchema.call(this);
		schema.fieldsNames.push("tips");
		schema.fieldsTypes.push(hxbit_PropTypeDesc.PArray(hxbit_PropTypeDesc.PString));
		schema.isFinal = hxbit_Serializer.isClassFinal(model_ver1_data__$CardProto_$179030_$11E_$CH_$BN091N_$brown_Process1.__clid);
		return schema;
	}
	,unserialize: function(__ctx) {
		model_ver1_game_define_CardText.prototype.unserialize.call(this,__ctx);
...
function model_ver1_data_CardProto_$179030_$11E_$CH_$BN091N_$brown_test() {
	var player1 = model_ver1_game_define_Define_PLAYER_A;
	var player2 = model_ver1_game_define_Define_PLAYER_B;
	var ctx = new model_ver1_game_define_Context();
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:106:","卡牌1在場");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:111:","敵軍G在場");
	var card2 = new tool_Card("2");
	card2.owner = player2;
	card2.protoId = "179030_11E_CH_BN091N_brown";
	card2.isTap = false;
	tool_Table_addCard(ctx.table,model_ver1_game_define_BaSyou_getCardStackId(model_ver1_game_define_BaSyou.Default(player2,model_ver1_game_define_BaSyouKeyword.GZone)),card2);
	if(model_ver1_game_alg_Cut_getTopCut(ctx).length != 0) {
		throw haxe_Exception.thrown("一開始堆疊中沒有效果");
	}
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:121:","卡牌横置");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:136:","選擇");
	model_ver1_game_alg_Context_setPlayerSelectionCardId(ctx,$require.id,[$require.tips[0]]);
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:138:","驗証支付");
	$require.action(ctx,runtime);
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:140:","解決效果");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:148:","結束一個turn");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_CH_BN091N_brown.hx:154:","再結束一個turn");
	model_ver1_game_alg_Context_sendEvent(ctx,model_ver1_game_define_Event.ChangePhase);
	var _g = [];
	var h = ctx.marks.h;
	var mark_h = h;
...
var model_ver1_data__$CardProto_$179030_$11E_$U_$VT186R_$purple_Process1 = function(id,cardIds) {
...
model_ver1_data__$CardProto_$179030_$11E_$U_$VT186R_$purple_Process1.prototype = $extend(model_ver1_game_define_CardText.prototype,{
	getRequires: function(ctx,runtime) {
		var req = new model_ver1_data_RequireUserSelectCard(this.getSubKey(0),"このターン中に場に出た敵軍ユニット１枚");
		req.tips = this.cardIds;
		return [req];
	}
	,action: function(ctx,runtime) {
		var cardId = runtime.getCardId();
	...
	,getSerializeSchema: function() {
		...
		schema.fieldsTypes.push(hxbit_PropTypeDesc.PArray(hxbit_PropTypeDesc.PString));
		schema.isFinal = hxbit_Serializer.isClassFinal(model_ver1_data__$CardProto_$179030_$11E_$U_$VT186R_$purple_Process1.__clid);
		return schema;
	}
	,unserialize: function(__ctx) {
		model_ver1_game_define_CardText.prototype.unserialize.call(this,__ctx);
...
function model_ver1_data_CardProto_$179030_$11E_$U_$VT186R_$purple_test() {
	...
	var player2Hand = new tool_CardStack(model_ver1_game_define_BaSyou_getCardStackId(model_ver1_game_define_BaSyou.Default(player2,model_ver1_game_define_BaSyouKeyword.TeHuTa)));
	ctx.table.cardStacks.h[player2Hand.id] = player2Hand;
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:98:","機體1在場");
	var card = new tool_Card("1");
	card.owner = player1;
	card.protoId = "179030_11E_U_VT186R_purple";
	tool_Table_addCard(ctx.table,model_ver1_game_define_BaSyou_getCardStackId(model_ver1_game_define_BaSyou.Default(player1,model_ver1_game_define_BaSyouKeyword.MaintenanceArea)),card);
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:103:","機體2這回合剛出場");
	var card2 = new tool_Card("2");
	card2.owner = player2;
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:108:","設置剛出場標記");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:115:","機體1出場事件");
	...
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:130:","選擇要回手的一個敵機");
	model_ver1_game_alg_Context_setPlayerSelectionCardId(ctx,$require.id,[$require.tips[0]]);
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:132:","驗証支付");
	$require.action(ctx,runtime);
	console.log("src/model/ver1/data/CardProto_179030_11E_U_VT186R_purple.hx:134:","解決效果");
	block.text.action(ctx,runtime);
...
function model_ver1_game_alg_Context_moveCard(ctx,cardId,from,to) {
	tool_Table_moveCard(ctx.table,cardId,model_ver1_game_define_BaSyou_getCardStackId(from),model_ver1_game_define_BaSyou_getCardStackId(to));
}
...
function model_ver1_game_alg_Context_getCardBaSyouAndAssertExist(ctx,cardId) {
	...
	if(_g._hx_index == 0) {
		...
		throw new haxe_Exception("card baSyou not found");
	}
}
...
function model_ver1_game_alg_Runtime_getRuntimeText(ctx) {
	...
	while(_g2 < _g3.length) {
		...
		++_g2;
		var _this;
		var _g = model_ver1_game_define_BaSyou_getBaSyou(v.id);
		var _g4 = _g.playerId;
...
function model_ver1_game_define_BaSyou_getCardStackId(obj) {
	...
	return "" + playerId + "@@@" + $hxEnums[kw.__enum__].__constructs__[kw._hx_index]._hx_name;
}
function model_ver1_game_define_BaSyou_getBaSyou(cardStackId) {
	try {
		var pair = cardStackId.split("@@@");
		var playerId = pair[0];
...
function model_ver1_test_Test_test_constantText() {
	...
	tool_Table_addCard(ctx.table,model_ver1_game_define_BaSyou_getCardStackId(model_ver1_game_define_BaSyou.Default("0",model_ver1_game_define_BaSyouKeyword.TeHuTa)),card);
...
function model_ver1_test_Test_test_getMarkEffects() {
	...
	var card = new tool_Card("0");
	card.protoId = "AddTextCardProto";
	tool_Table_addCard(ctx.table,model_ver1_game_define_BaSyou_getCardStackId(model_ver1_game_define_BaSyou.Default("0",model_ver1_game_define_BaSyouKeyword.MaintenanceArea)),card);
...
var model_ver1_game_define_Define_PLAYER_B = "PLAYER_B";
