...
class MatchLevels {
  ...
  static function jiceLuoshiBasic(game:SimpleGame):MatchBootstrap {
    var attacker = new SimpleMonarch("m-atk", 0, 0, 500, 80);
    attacker.addGeneral(game.createGeneral("g-might-high", "m-atk", 1, 50, 1, 1));
    attacker.addGeneral(game.createGeneral("g-might-low", "m-atk", 1, 20, 1, 1));
    var defender = new SimpleMonarch("m-def", 1, 0, 100, 200);
  ...
  static function jiceLuoshiCeiling(game:SimpleGame):MatchBootstrap {
    var atk = new SimpleMonarch("atk-ceiling", 0, 0, 1, 0);
    atk.addGeneral(game.createGeneral("g-ceil", "atk-ceiling", 1, 5, 1, 1));
    var d = new SimpleMonarch("m-33", 1, 0, 33, 0);
    var tiles:Array<ITile> = [game.createTile(0, Plain)];
    var board = game.createBoard(tiles);
  ...
  static function jiceStagingThreeGenerals(game:SimpleGame):MatchBootstrap {
    var atk = new SimpleMonarch("atk", 0, 0, 200, 0);
    atk.addGeneral(game.createGeneral("wa", "atk", 1, 40, 1, 1));
    atk.addGeneral(game.createGeneral("wb", "atk", 1, 10, 1, 1));
    atk.addGeneral(game.createGeneral("wc", "atk", 1, 25, 1, 1));
    var def = new SimpleMonarch("def", 1, 0, 80, 10);
    var tiles:Array<ITile> = [game.createTile(0, Plain)];
    var board = game.createBoard(tiles);
  ...
  static function sliceMinimalSoloRing2(game:SimpleGame):MatchBootstrap {
    var monarch = game.createMonarch("m-a", 0, 0);
  ...
  static function sliceJiceTwoMonarchsPlain(game:SimpleGame):MatchBootstrap {
    var atk = new SimpleMonarch("m-a", 0, 0, 10, 0);
    atk.addGeneral(game.createGeneral("g-ts", "m-a", 1, 15, 1, 1));
    var def = new SimpleMonarch("m-b", 1, 0, 50, 0);
    var tiles:Array<ITile> = [game.createTile(0, Plain)];
    var board = game.createBoard(tiles);
    return {board: board, monarchs: [atk, def], activeMonarchId: atk.id()};
  }
