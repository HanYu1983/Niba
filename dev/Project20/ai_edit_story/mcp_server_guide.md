...
4. **選定生成路線**（`r2v`／`t2v`／`i2v`），再依下方對應小節寫提示詞與備圖。
   同一 story 可混用，但合併前解析度必須統一；路線選定後元素 `type` 與 `refs` 寫法不同，
   不要混用另一路線的欄位習慣。

### 4A. r2v 方法（參考圖驅動）

適用：已有穩定角色／場景 t2i 素材，要用參考圖鎖外觀與場景一致性。

1. **用好參考圖**：這格分鏡中出現的每一個角色／場景，若已有對應 t2i 素材元素，
   一律加進 `refs`（放該 t2i 元素 id，提交時自動解析成圖片路徑）。
   r2v 最多 9 張；一張主角肖像優先放 `ref_image_0`。
   未提供的 `ref_image_N` 不要帶入——MCP 層會刪除該節點，避免殘留污染。
   **所有參考圖（t2i 素材）一律用 Z-Image (zit) 生成**（見 §五），不用 SDXL。
2. **寫六欄位 r2v 提示詞**（詳見 `video_prompt_guide.md` §3.5）：
...
適用：不必（或暫時沒有）準備參考圖；希望畫面細節由文字盡量發揮，
而不必擔心被參考圖外觀／構圖牽制。

1. **不備參考圖**：元素 `type` 設為 `t2v`，`refs` 留空（或不帶）。提交走 `gen_t2v_video`。
2. **角色一致性改由文字定錨**：
   - 在提示詞中寫死穩定的**身體外觀**（年齡感、體型、髮型髮色、臉部特徵、種族／東亞面孔等），
     全片各格重複同一套錨點，避免漂成另一人。
...
| # | 原則 | 為什麼 |
|---|------|--------|
| 1 | `summary` 只寫英文名詞片語，**不寫任何中文完整句**（r2v）；t2v/i2v 則勿在三欄位中塞中文完整句於 `<d>` 外 | 模型會把 `<d>` 外的中文完整句當成可朗讀內容，覆蓋掉 `<d>` 內內容 |
| 2 | `summary` 不出現台詞關鍵字（r2v）；t2v/i2v 的鏡頭描述同樣避免複述台詞語意 | 避免對白用詞（「背叛」「懷疑」等）被抓去唸 |
