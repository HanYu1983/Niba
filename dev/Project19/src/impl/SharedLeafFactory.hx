 ...
 *
 * 設計動機:
 *   原本每個 goal 都自帶一份 LeafFactory closure (例: createWalkWaypointsGoal 回傳的 built.factory),
 *   呼叫端要自行記住「哪個 goal 配哪個 factory」, 且 closure 內可能 capture 大量 per-instance 參數,
 *   無法跨 actor / 跨目標重用。
 *
 *   改進方式:
 *     - lifecycle 改為「無狀態的純函式集合」, 所有 per-instance 參數寫進 leafState
 *       (例: MoveToPointGoal 把 target / arrivalDistance 放到 leafState)
 *     - 一個全域 SharedLeafFactory 統一登記所有 leaf 名稱對應的 lifecycle
 *     - runtime 引擎 (runFrame) 只需要這一個 factory, 不再需要每個 goal 各自帶
 *
 * 註冊新 leaf:
